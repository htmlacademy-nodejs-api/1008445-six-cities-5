...
import { City, NewOrEditOffer, Offer } from '../../types/types';
...
const getImages = (
  ...
  for (const [formFieldName, file] of entries) {
    if (formFieldName.startsWith(FormFieldName.image) && file instanceof File && file.name) {
      enteredImages.push(file);
    }
  }
...
const OfferForm = <T extends Offer | NewOrEditOffer>({
  offer,
  onSubmit,
    ...
    >
      ...
      <fieldset className="images-fieldset">
        {images.map((image, index) => (
          ...
          <div key={index} className="form__input-wrapper">
